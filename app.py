...
def extract_pharmacopeia_folder(metadata: dict) -> str:
    """
    Extract the pharmacopoeia label for a chunk.

    Priority order:
      1. 'pharmacopeia' field — written directly by the updated uploader
         e.g. metadata['pharmacopeia'] = 'USP'
      2. Full 'source' path — parse the first folder after MyPDFDatabase
         e.g. /content/drive/MyDrive/MyPDFDatabase/USP/Appendix X/file.pdf -> 'USP'
      3. Filename-only 'source' — last resort, return 'UNKNOWN'
    """
    # Priority 1: direct field from updated uploader
    pharma = metadata.get("pharmacopeia", "")
    if pharma:
        return pharma.upper()

    # Priority 2: parse full path
    ROOT_MARKER = "MyPDFDatabase"
    source = metadata.get("source", "")
    parts  = source.replace("\\", "/").split("/")
    if ROOT_MARKER in parts:
        idx = parts.index(ROOT_MARKER)
        if idx + 1 < len(parts) - 1:          # folder exists after root and before filename
...
def assemble_context(docs: List) -> Tuple[str, List[Dict]]:
    """
    Assemble retrieval context grouped by pharmacopoeia (USP / BP / EDQM …).
    Each group is kept strictly separate so the LLM never mixes sources.
    """
    ...
    for idx, d in enumerate(docs, 1):
        ...
        # Use source_file field if available (uploader stores display name there)
        source_file = meta.get('source_file') or raw_source.split('/')[-1]
        page        = meta.get('page', 0) + 1
        subfolder   = meta.get('subfolder', '')          # e.g. "Appendix X"
        folder      = extract_pharmacopeia_folder(meta)

        grouped[folder].append((idx, source_file, subfolder, page, d.page_content))
        refs.append({"index": idx, "source": source_file, "page": page,
                     "folder": folder, "subfolder": subfolder})

    # Build context — one clearly labelled block per pharmacopoeia
    sections = []
    for folder, chunks in grouped.items():
        ...
        chunk_blocks = []
        for idx, source_file, subfolder, page, content in chunks:
            loc = f"{subfolder}/{source_file}" if subfolder else source_file
            chunk_blocks.append(f"[{idx}. 📄 {loc} | Page {page}]\n{content}")
            ...
            if show_sources and metadata_refs:
                ...
                with st.expander("📚 Sources Referenced", expanded=False):
                    ...
                    for ref in metadata_refs:
                        folder_label = ref.get('folder', '')
                        sub_label    = f" › {ref['subfolder']}" if ref.get('subfolder') else ""
                        st.write(f"**[{ref['index']}]** [{folder_label}{sub_label}] {ref['source']} (Page {ref['page']})")
